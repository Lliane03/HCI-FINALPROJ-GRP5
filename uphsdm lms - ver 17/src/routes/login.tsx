...
const DEPARTMENTS = ['Grade School', 'Junior High School', 'Senior High School', 'College'] as const
const COLLEGE_DEPARTMENTS = ['College of Computer Studies', 'College of Engineering', 'College of Architecture'] as const
const GRADE_SCHOOL_LEVELS = ['Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5', 'Grade 6'] as const
const JUNIOR_HIGH_LEVELS = ['Grade 7', 'Grade 8', 'Grade 9', 'Grade 10'] as const
const SENIOR_HIGH_LEVELS = [
  'Grade 11 - STEM', 'Grade 12 - STEM',
  'Grade 11 - ABM', 'Grade 12 - ABM',
  'Grade 11 - HUMSS', 'Grade 12 - HUMSS',
  'Grade 11 - GAS', 'Grade 12 - GAS',
  'Grade 11 - ICT', 'Grade 12 - ICT',
] as const

function getGradeLevels(department: string): readonly string[] | null {
  if (department === 'Grade School') return GRADE_SCHOOL_LEVELS
  if (department === 'Junior High School') return JUNIOR_HIGH_LEVELS
  if (department === 'Senior High School') return SENIOR_HIGH_LEVELS
  return null
}
...
function Login() {
  ...
  function validateRegister() {
    ...
    if (getGradeLevels(regDepartment) && !regGradeLevel.trim()) {
      errs.gradeLevel = 'Please select your grade level.'
    }
    return errs
  }
            ...
            <>
              ...
              <form onSubmit={handleRegister} className="p-6 space-y-4" noValidate>
                ...
                <div>
                  ...
                  <select
                    value={regDepartment}
                    onChange={(e) => { setRegDepartment(e.target.value); setRegCollegeType(''); setRegGradeLevel(''); if (errors.department) setErrors((p) => ({ ...p, department: undefined as any })) }}
                ...
                {regDepartment === 'College' && (
                  <div>
                    <label className="flex items-center gap-1 text-sm font-medium text-gray-700 mb-1.5">
                      ...
                      <Tooltip text="Select which College department you teach in" position="right" />
                    </label>
                    <select
                    ...
                    >
                      ...
                      {COLLEGE_DEPARTMENTS.map((c) => <option key={c} value={c}>{c}</option>)}
                    </select>
                    {errors.collegeType && (
                ...
                {/* Grade Level (conditional for Grade School / JHS / SHS) */}
                {getGradeLevels(regDepartment) && (
                  <div>
                    <label className="flex items-center gap-1 text-sm font-medium text-gray-700 mb-1.5">
                      Grade Level <span className="text-red-500">*</span>
                      <Tooltip text="Select the grade level you teach" position="right" />
                    </label>
                    <select
                      value={regGradeLevel}
                      onChange={(e) => { setRegGradeLevel(e.target.value); if (errors.gradeLevel) setErrors((p) => ({ ...p, gradeLevel: undefined as any })) }}
                      className={`w-full px-3.5 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 bg-white transition-colors ${
                        errors.gradeLevel ? 'border-red-400 focus:ring-red-300 bg-red-50' : 'border-gray-300 focus:ring-[#D4AF37]'
                      }`}
                    >
                      <option value="">Select your grade level...</option>
                      {getGradeLevels(regDepartment)!.map((g) => <option key={g} value={g}>{g}</option>)}
                    </select>
                    {errors.gradeLevel && (
                      <div className="flex items-start gap-1.5 mt-1.5 text-xs text-red-600"><AlertCircle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />{errors.gradeLevel}</div>
                    )}
                  </div>
                )}
