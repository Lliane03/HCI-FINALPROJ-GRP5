...
export function emptyPeriodGrades(): PeriodGrades {
  return {
    ...
    quizzes: Array.from({ length: 4 }, () => ({ rs: 0, qe: 0 })),
    ra: [0, 0, 0, 0],
    project: [0, 0, 0, 0],
...
export function computeGrades(student: Student) {
  ...
  // Quizzes (QE average): 30%
  const validQE = student.quizzes.filter((q) => q.qe > 0)
  const quizAvg =
    validQE.length > 0
      ? validQE.reduce((s, q) => s + q.qe, 0) / validQE.length
      : 0
  const quizWeighted = quizAvg * 0.3
  ...
  const raAvg =
    validRA.length > 0
      ...
      : 0
  const raWeighted = raAvg * 0.5

  // Class Performance (total weights: 10+10+30+50 = 100%)
  const classPerformance = attWeighted + cpWeighted + quizWeighted + raWeighted

  // Project grade (shown separately, 10%)
  ...
  // Finale: CLASS PERFORMANCE × 70% + Major × 30%
  const classGrade70 = classPerformance * 0.7
  const tps = student.majorExam.tps || student.majorExam.raw || 0
  const majorWeighted = tps * 0.3
  const finaleRaw = classGrade70 + majorWeighted
