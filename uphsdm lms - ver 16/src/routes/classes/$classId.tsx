...
function ClassDetail() {
  ...
  function updateField(id: string, path: string[], value: number | string) {
    ...
    // Auto-compute TPS from raw
    if (path.includes('raw')) {
      periodData.majorExam.tps = periodData.majorExam.raw
    }
          ...
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Class Performance (70%) + Major Exam (30%) = Finale Grade
                <Tooltip text="Class Performance = Attendance(10%) + Class Participation(10%) + Quizzes Avg(30%) + RA Avg(50%). Then × 70%. Major Exam × 30%. Sum = Finale Grade." position="bottom" />
              </p>
            </div>
              ...
              <table className="text-xs border-collapse min-w-max w-full">
                <thead>
                  <tr className="bg-[#7a1e33] text-white">
                    ...
                    <th className="border border-[#c8a45d] px-2 py-1 text-center bg-[#5e1724]" colSpan={4}>MAJOR EXAM 30%</th>
                    <th className="border border-[#c8a45d] px-2 py-1 text-center bg-[#3d0f18]" rowSpan={3}>FINALE<br/>GRADE</th>
                  ...
                  <tr className="bg-[#96283f] text-white">
                    ...
                    <th className="border border-[#d4a94a] px-2 py-1 text-center" colSpan={2}>CLASS PART.</th>
                    <th className="border border-[#d4a94a] px-2 py-1 text-center" colSpan={2}>QUIZZES/EXERCISES Avg</th>
                    <th className="border border-[#d4a94a] px-2 py-1 text-center" colSpan={2}>RA/RESEARCH Avg</th>
                    <th className="border border-[#e0b963] px-2 py-1 text-center" colSpan={2}>FINALE</th>
                    <th className="border border-[#e0b963] px-2 py-1 text-center" colSpan={2}>CLASS PERF.</th>
                  </tr>
                  <tr className="bg-[#a83d54] text-white text-[10px]">
                    ...
                    <th className="border border-[#e0b963] px-2 py-1 text-center">Raw</th>
                    <th className="border border-[#e0b963] px-2 py-1 text-center">TPS</th>
                    <th className="border border-[#e0b963] px-2 py-1 text-center">Score</th>
                    <th className="border border-[#e0b963] px-2 py-1 text-center">70%</th>
                  </tr>
                </thead>
                <tbody>
                  {displayStudents.map((student, idx) => {
                    ...
                    return (
                      <tr key={student.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        {/* Class Participation */}
                        <td className="border border-gray-200 px-1 py-1 bg-[#fdf0c4] min-w-[52px]">
                          <NumInput value={student.classParticipation} onChange={(v) => updateField(student.id, ['classParticipation'], v)} />
                        </td>
                        <td className="border border-gray-200 px-1 py-1 text-center bg-[#faf3e0]">
                          <span className="text-[#7a1e33] font-semibold">{g.cpWeighted.toFixed(2)}</span>
                        </td>

                        {/* Quiz Average */}
                        <td className="border border-gray-200 px-1 py-1 text-center bg-[#faf3e0] min-w-[48px]">
                          <div className="text-[#5e1724] font-medium">{g.quizAvg.toFixed(2)}</div>
                          <QuizPopover student={student} updateField={updateField} />
                        </td>
                        <td className="border border-gray-200 px-1 py-1 text-center bg-[#faf3e0]">
                        ...
                        <td className="border border-gray-200 px-1 py-1 text-center bg-[#faf3e0] min-w-[48px]">
                          ...
                          <RAPopover student={student} updateField={updateField} />
                        </td>
                        <td className="border border-gray-200 px-1 py-1 text-center bg-[#faf3e0]">
                        ...
                        <td className="border border-gray-200 px-1 py-1 bg-[#fdf0c4] min-w-[52px]">
                          <NumInput
                            value={student.majorExam.raw}
                            onChange={(v) => updateField(student.id, ['majorExam', 'raw'], v)}
                          />
                        </td>
                        {/* TPS */}
                        <td className="border border-gray-200 px-1 py-1 text-center bg-[#faf3e0]">
                          <span className="text-[#7a1e33]">{student.majorExam.tps || student.majorExam.raw}</span>
                        </td>
                        {/* Major weighted */}
                        <td className="border border-gray-200 px-1 py-1 text-center bg-[#faf3e0]">
                        ...
                        <td className="border border-gray-200 px-1 py-1 text-center bg-[#faf3e0]">
                          <span className="text-[#7a1e33] font-semibold">{g.classGrade70.toFixed(2)}</span>
                        </td>

                        {/* Finale Grade */}
                        <td className={`border border-gray-200 px-2 py-1 text-center font-bold bg-[#f5e6c8] ${g.finaleGrade >= 75 ? 'text-emerald-700' : 'text-red-600'}`}>
                          {g.finaleGrade > 0 ? g.finaleGrade : '—'}
                        </td>
                        {/* GWA */}
                        <td className="border border-gray-200 px-2 py-1 text-center bg-[#faf3e0] text-[#7a1e33] font-semibold">
                          {g.finaleGrade > 0 ? g.gradesLabel : '—'}
                        </td>
                        {/* Remarks */}
                        <td className={`border border-gray-200 px-2 py-1 text-center text-[10px] font-bold ${
                          g.remarks === 'PASSED' ? 'text-emerald-600' : 'text-red-500'
                        }`}>
...
// Quiz popover
function QuizPopover({ student, updateField }: { student: Student; updateField: (id: string, path: string[], value: any) => void }) {
  const [open, setOpen] = useState(false)
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="text-[10px] text-[#7a1e33] hover:underline block w-full text-center font-medium">
      ...
      {open && (
        <div className="absolute z-40 top-full left-0 bg-[#faf3e0] border-2 border-[#c8a45d] rounded-xl shadow-xl p-3 min-w-[220px]">
          <div className="flex items-center justify-between mb-2 pb-2 border-b border-[#e8d9b0]">
            <span className="text-xs font-bold text-[#7a1e33] uppercase tracking-wide">Quizzes · QE1–QE4</span>
            <button onClick={() => setOpen(false)} className="text-[#7a1e33] hover:bg-[#f5e6c8] rounded p-0.5"><X className="w-3 h-3" /></button>
          </div>
          {student.quizzes.map((q, qi) => (
            <div key={qi} className="flex items-center gap-2 py-1">
              <span className="text-xs font-semibold text-[#7a1e33] w-8">QE{qi + 1}</span>
              <div className="flex items-center gap-1">
                <span className="text-[10px] text-[#7a1e33]/70 font-medium">RS</span>
                <input
                  type="number" min={0} max={100}
                  value={q.rs || ''}
                  onChange={(e) => {
                    const upd = [...student.quizzes]
                    upd[qi] = { ...upd[qi], rs: parseFloat(e.target.value) || 0 }
                    updateField(student.id, ['quizzes'], upd)
                  }}
                  className="w-12 text-xs border border-[#d4a94a] rounded px-1 py-0.5 bg-[#fdf0c4] text-center focus:outline-none focus:ring-1 focus:ring-[#c8a45d]"
                />
                <span className="text-[10px] text-[#7a1e33]/70 font-medium">QE</span>
                <input
                  type="number" min={0} max={100}
                  value={q.qe || ''}
                  onChange={(e) => {
                    const upd = [...student.quizzes]
                    upd[qi] = { ...upd[qi], qe: parseFloat(e.target.value) || 0 }
                    updateField(student.id, ['quizzes'], upd)
                  }}
                  className="w-12 text-xs border border-[#d4a94a] rounded px-1 py-0.5 bg-[#fdf0c4] text-center focus:outline-none focus:ring-1 focus:ring-[#c8a45d]"
                />
          ...
          <div className="mt-2 pt-2 border-t border-[#e8d9b0] text-[10px] text-[#7a1e33]/70 font-medium">RS = Raw Score · QE = Quiz Equivalent</div>
        </div>
      )}
    </div>
  )
...
// RA popover
function RAPopover({ student, updateField }: { student: Student; updateField: (id: string, path: string[], value: any) => void }) {
  const [open, setOpen] = useState(false)
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="text-[10px] text-[#7a1e33] hover:underline block w-full text-center font-medium">
      ...
      {open && (
        <div className="absolute z-40 top-full left-0 bg-[#faf3e0] border-2 border-[#c8a45d] rounded-xl shadow-xl p-3 min-w-[200px]">
          <div className="flex items-center justify-between mb-2 pb-2 border-b border-[#e8d9b0]">
            <span className="text-xs font-bold text-[#7a1e33] uppercase tracking-wide">RA / Research · RA1–RA4</span>
            <button onClick={() => setOpen(false)} className="text-[#7a1e33] hover:bg-[#f5e6c8] rounded p-0.5"><X className="w-3 h-3" /></button>
          </div>
          {student.ra.map((r, ri) => (
            <div key={ri} className="flex items-center gap-2 py-1">
              <span className="text-xs font-semibold text-[#7a1e33] w-10">RA{ri + 1}</span>
              <input
                type="number" min={0} max={100}
                value={r || ''}
                onChange={(e) => {
                  const upd = [...student.ra]
                  upd[ri] = parseFloat(e.target.value) || 0
                  updateField(student.id, ['ra'], upd)
                }}
                className="w-16 text-xs border border-[#d4a94a] rounded px-1 py-0.5 bg-[#fdf0c4] text-center focus:outline-none focus:ring-1 focus:ring-[#c8a45d]"
              />
            </div>
          ))}
          <div className="mt-2 pt-2 border-t border-[#e8d9b0] text-[10px] text-[#7a1e33]/70 font-medium">Research Activity weighted at 50%</div>
        </div>
      )}
    </div>
  )
...
function StudentScoreCard({ student, idx, updateField }: {
  ...
  const [totals, setTotals] = useState<{
    ...
    majorExam: number
  }>({ quizzes: {}, ra: {}, project: {}, majorExam: 100 })
  const g = computeGrades(student)
        ...
        <div className="border-t-2 border-[#c8a45d] p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 bg-[#faf3e0]/40">
          ...
          <div className="bg-white rounded-lg border border-[#e8d9b0] p-3">
            ...
            <div className="mt-2 text-xs font-semibold text-[#7a1e33] bg-[#f5e6c8] border border-[#e8d9b0] rounded px-2 py-1.5">
              Grade: <span className="font-bold">{computeAttendanceGrade(student.attendance).toFixed(2)}</span> → <span className="font-bold">{g.attWeighted.toFixed(2)}</span> <span className="text-[#7a1e33]/70 text-[10px]">(10%)</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-[#e8d9b0] p-3">
            <div className="text-[10px] font-bold text-[#7a1e33] mb-2 uppercase tracking-wider flex items-center gap-1.5 pb-1.5 border-b border-[#e8d9b0]">
              <ClipboardList className="w-3 h-3" /> Class Part. &amp; Quizzes
            </div>
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xs text-[#7a1e33] font-medium w-20">Class Part.</span>
              <input
                type="number" min={0} max={100} value={student.classParticipation || ''}
                onChange={(e) => updateField(student.id, ['classParticipation'], parseFloat(e.target.value) || 0)}
                className="w-16 text-xs border border-[#d4a94a] rounded px-1.5 py-1 bg-[#fdf0c4] text-center focus:outline-none focus:ring-1 focus:ring-[#c8a45d]"
              />
              <span className="text-xs text-[#7a1e33] font-semibold">→ {g.cpWeighted.toFixed(2)}</span>
            </div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[10px] font-bold text-[#7a1e33] uppercase tracking-wider">Quizzes / Exercises</span>
              <button
                type="button"
                onClick={() => updateField(student.id, ['quizzes'], [...student.quizzes, { rs: 0, qe: 0 }])}
            ...
            {student.quizzes.map((q, qi) => (
              <div key={qi} className="flex items-center gap-2 mb-1.5">
                ...
                <div className="flex items-center gap-1 flex-1">
                  ...
                  <input type="number" min={0} value={totals.quizzes[qi] ?? 100}
                    onChange={(e) => setTotals({ ...totals, quizzes: { ...totals.quizzes, [qi]: parseFloat(e.target.value) || 0 } })}
                    className="w-14 text-xs border border-[#d4a94a] bg-[#fdf0c4] rounded px-1 py-1 text-center focus:outline-none focus:ring-1 focus:ring-[#c8a45d]"
                    placeholder="Total" />
                </div>
                <button
            ...
            <div className="mt-2 text-xs font-semibold text-[#7a1e33] bg-[#f5e6c8] border border-[#e8d9b0] rounded px-2 py-1.5">
              Quiz Avg: <span className="font-bold">{g.quizAvg.toFixed(2)}</span> → <span className="font-bold">{g.quizWeighted.toFixed(2)}</span> <span className="text-[#7a1e33]/70 text-[10px]">(30%)</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-[#e8d9b0] p-3">
            <div className="text-[10px] font-bold text-[#7a1e33] mb-2 uppercase tracking-wider flex items-center gap-1.5 pb-1.5 border-b border-[#e8d9b0]">
              <BookOpen className="w-3 h-3" /> RA, Project &amp; Major Exam
            </div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[10px] font-bold text-[#7a1e33] uppercase tracking-wider">Research Assignments (RA)</span>
              <button
                type="button"
            ...
            {student.ra.length === 0 && (
              <div className="text-[10px] text-[#7a1e33]/60 italic py-2 text-center">No research assignments yet.</div>
            )}
            {student.ra.map((r, ri) => (
            ...
            <div className="mb-3 mt-2 text-xs font-semibold text-[#7a1e33] bg-[#f5e6c8] border border-[#e8d9b0] rounded px-2 py-1.5">
              RA Avg: <span className="font-bold">{g.raAvg.toFixed(2)}</span> → <span className="font-bold">{g.raWeighted.toFixed(2)}</span> <span className="text-[#7a1e33]/70 text-[10px]">(50%)</span>
            </div>
            <div className="flex items-center justify-between mt-2 mb-1.5">
              <span className="text-[10px] font-bold text-[#7a1e33] uppercase tracking-wider">Projects</span>
              <button
                type="button"
                onClick={() => updateField(student.id, ['project'], [...student.project, 0])}
                className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold text-[#7a1e33] bg-[#fdf0c4] border border-[#d4a94a] rounded hover:bg-[#fae4a0] transition-colors"
              >
                <Plus className="w-3 h-3" /> Add
              </button>
            ...
            {student.project.length === 0 && (
              <div className="text-[10px] text-[#7a1e33]/60 italic py-2 text-center">No projects yet.</div>
            )}
            {student.project.map((p, pi) => (
              <div key={pi} className="flex items-center gap-2 mb-1.5">
                <span className="text-xs text-[#7a1e33] font-medium w-10">#{pi + 1}</span>
                <div className="flex items-center gap-1 flex-1">
                  <input type="number" min={0} value={p || ''}
                    onChange={(e) => { const upd = [...student.project]; upd[pi] = parseFloat(e.target.value) || 0; updateField(student.id, ['project'], upd) }}
                    className="w-14 text-xs border border-[#d4a94a] bg-[#fdf0c4] rounded px-1 py-1 text-center focus:outline-none focus:ring-1 focus:ring-[#c8a45d]"
                    placeholder="Score" />
                  <span className="text-[#7a1e33] font-bold text-xs">/</span>
                  <input type="number" min={0} value={totals.project[pi] ?? 100}
                    onChange={(e) => setTotals({ ...totals, project: { ...totals.project, [pi]: parseFloat(e.target.value) || 0 } })}
                    className="w-14 text-xs border border-[#d4a94a] bg-[#fdf0c4] rounded px-1 py-1 text-center focus:outline-none focus:ring-1 focus:ring-[#c8a45d]"
                    placeholder="Total" />
                </div>
                <button
                  type="button"
                  onClick={() => updateField(student.id, ['project'], student.project.filter((_, i) => i !== pi))}
                  className="p-1 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded transition-colors ml-auto"
                  title={`Remove project #${pi + 1}`}
                >
                  <Trash2 className="w-3 h-3" />
                </button>
              </div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <div className="flex items-center gap-1">
                <input type="number" min={0} value={student.majorExam.raw || ''}
                  ...
                  placeholder="Score" />
                <span className="text-[#7a1e33] font-bold text-xs">/</span>
                <input type="number" min={0} value={totals.majorExam || ''}
                  onChange={(e) => setTotals({ ...totals, majorExam: parseFloat(e.target.value) || 0 })}
                  className="w-14 text-xs border border-[#d4a94a] bg-[#fdf0c4] rounded px-1 py-1 text-center focus:outline-none focus:ring-1 focus:ring-[#c8a45d]"
                  placeholder="Total" />
              </div>
              <span className="text-xs text-[#7a1e33] font-semibold">→ {g.majorWeighted.toFixed(2)} <span className="text-[#7a1e33]/70 text-[10px]">(30%)</span></span>
            </div>
